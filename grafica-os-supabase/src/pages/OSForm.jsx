...
export default function OSForm() {
  ...
  const [form, setForm] = useState({
    ...
    vendedor: 'EF COMÉRCIO DE PAPÉIS LTDA',
    prazo_entrega: '',
  ...
  useEffect(() => {
    Promise.all([api.getClientes(), api.getProdutos()]).then(([c, p]) => {
      setClientes(c);
      setProdutos(p.filter(x => x.ativo === true || x.ativo === 'true'));
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          <div className="form-row-2">
            ...
            <div className="form-group">
              <label className="form-label">Emitido por</label>
              <input className="form-control" value="EF COMÉRCIO DE PAPÉIS LTDA" readOnly style={{ background: 'var(--bg)', color: 'var(--text-muted)', cursor: 'default' }} />
            </div>
          </div>
